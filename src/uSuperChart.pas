...
type
  ...
  private
    ...
    Line: TLine;
    LineCircle: TCircle;
    LabelContent, LabelValue, LabelArg: TLabel;
    BarCount: Integer;
    MaxValue: double;
    CountComp: Integer;
    ...
    procedure AddBar(vl: double; field, content: string);
  ...
  public
    ...
    procedure LoadFromJSON(jsonString: string; var error: string); overload;
    procedure LoadFromJSON(jsonString, aField, aValue, aContent: string;
      var error: string); overload;
    ...
    property FontColorValues: Cardinal read FFontColorValues
      write FFontColorValues;
    ...
    property FontSizeArgument: Integer read FFontSizeArgument
      write FFontSizeArgument;
    property FontColorArgument: Cardinal read FFontColorArgument
      write FFontColorArgument;
...
procedure TSuperChart.BarMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Single);
var
  lbl: TLabel;
begin
  ...
  begin
    ...
    lbl := TRectangle(Sender).FindComponent(TRectangle(Sender).TagString)
      as TLabel;
...
begin
  ...
  LineCircle.Position.X := CountComp * spaceBetweenPoints -
    Trunc(LineCircle.Width / 2);
  ...
  LabelValue.StyledSettings := LabelValue.StyledSettings -
  ...
  LabelArg.Position.X := (CountComp * spaceBetweenPoints -
    Trunc(LineCircle.Width / 2)) - Trunc(spaceBetweenPoints / 2) + 7;
  LabelArg.Position.Y := FLayout.Height - 25;
  LabelArg.Margins.Bottom := -20;
  LabelArg.TextSettings.HorzAlign := TTextAlign.Center;

  LabelArg.StyledSettings := LabelArg.StyledSettings - [TStyledSetting.Size,
...
procedure TSuperChart.AddBar(vl: double; field, content: string);
var
  porc: double;
begin
  inc(CountComp);
  ...
  begin
    ...
    if (FRoundedBotton and FRoundedTop) then
      Bar.Corners := [TCorner.TopLeft, TCorner.TopRight, TCorner.BottomLeft,
        TCorner.BottomRight]
    else if (FRoundedBotton) then
  ...
  if FChartType = HorizontalBar then
    Bar.AnimateFloat('Height', Trunc(BarBackground.Height * porc), 0.7,
      TAnimationType.InOut, TInterpolationType.Cubic)
  else if FChartType = VerticalBar then
    Bar.AnimateFloat('Width', Trunc(BarBackground.Width * porc), 0.7,
      TAnimationType.InOut, TInterpolationType.Cubic);
  ...
  LabelContent := TLabel.Create(Bar);
  LabelContent.Parent := Bar;
  LabelContent.Text := content;

  if FChartType = HorizontalBar then
  begin
    LabelContent.Align := TAlignLayout.Client;
    LabelContent.Margins.Bottom := -20;
    LabelContent.TextSettings.HorzAlign := TTextAlign.Center;
  end
  else if FChartType = VerticalBar then
  begin
    LabelContent.Align := TAlignLayout.Client;
    LabelContent.Margins.Left := -40;
    LabelContent.TextSettings.HorzAlign := TTextAlign.Leading;
  end;

  LabelContent.StyledSettings := LabelContent.StyledSettings - [TStyledSetting.Size,
    TStyledSetting.FontColor];
  LabelContent.Font.Size := FFontSizeArgument;
  LabelContent.Height := 20;
  LabelContent.FontColor := FFontColorArgument;
  ...
  if FChartType = HorizontalBar then
  begin
    LabelContent.Align := TAlignLayout.Bottom;
    LabelContent.Margins.Bottom := -20;
    LabelContent.TextSettings.HorzAlign := TTextAlign.Center;
  end
  else if FChartType = VerticalBar then
  begin
    LabelContent.Align := TAlignLayout.Left;
    LabelContent.Margins.Left := -40;
    LabelContent.TextSettings.HorzAlign := TTextAlign.Leading;
  end;
...
begin
  ...
  try
    jsonArray := TJSONObject.ParseJSONValue(TEncoding.UTF8.GetBytes(jsonString),
      0) as TJsonArray;
  except
...
procedure TSuperChart.LoadFromDataset(ds: TDataSet;
  FieldValue, FieldArgument: string; var error: string);
begin
  error := '';
  ClearChart;
  ...
  while NOT ds.Bof do
  begin
    AddBar(ds.FieldByName(FieldValue).AsFloat, ds.FieldByName(FieldArgument)
      .AsString, '');
...
procedure TSuperChart.LoadFromJSON(jsonString, aField, aValue, aContent: string;
  var error: string);
var
  jsonArray: TJsonArray;
  X: Integer;
...
begin
  ...
  for X := jsonArray.Size - 1 downto 0 do
    if MaxValue < jsonArray.Get(X).GetValue<double>(aValue) then
      MaxValue := jsonArray.Get(X).GetValue<double>(aValue);

  MaxValue := MaxValue * 1.1;

  if (FChartType = HorizontalBar) or (FChartType = VerticalBar) then
  begin
    for X := jsonArray.Count - 1 downto 0 do
    begin
      AddBar(jsonArray.Get(X).GetValue<double>(aValue),
        jsonArray.Get(X).GetValue<string>(aField), jsonArray.Get(X).GetValue<string>(aContent));
    end;
  end
  else if (FChartType = Lines) then
  begin
    ...
    for X := 0 to jsonArray.Size - 2 do
      AddLine(jsonArray.Get(X).GetValue<double>(aValue),
        jsonArray.Get(X + 1).GetValue<double>(aValue));

    CountComp := 0;
    for X := 0 to jsonArray.Size - 1 do
      AddLinePoint(jsonArray.Get(X).GetValue<double>(aValue),
        jsonArray.Get(X).GetValue<string>(aField));
